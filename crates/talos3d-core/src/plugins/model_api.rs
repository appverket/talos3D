...
fn handle_instantiate_recipe(
    ...
    // resolvable ObligationSet, so the result must say what was created — a
    // bare error reads as "nothing happened" and a blind retry duplicates
    // geometry. (Side-effect-producing gate blocks already arrive as Ok with
    // `promotion_blocked` set from promote_refinement_structured; this match
    // arm covers blocks where only the root anchor was created.)
