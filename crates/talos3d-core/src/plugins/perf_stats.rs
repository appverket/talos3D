...
fn begin_perf_frame(time: Res<Time>, mut perf_stats: ResMut<PerfStats>) {
    ...
    perf_stats.outliner_build_ms = 0.0;
    perf_stats.outliner_draw_ms = 0.0;
    perf_stats.layers_build_ms = 0.0;
    perf_stats.layers_draw_ms = 0.0;
...
pub fn add_outliner_build_time(perf_stats: &mut PerfStats, elapsed: Duration) {
    perf_stats.outliner_build_ms += elapsed.as_secs_f32() * 1000.0;
}

pub fn add_outliner_draw_time(perf_stats: &mut PerfStats, elapsed: Duration) {
    perf_stats.outliner_draw_ms += elapsed.as_secs_f32() * 1000.0;
}

pub fn add_layers_build_time(perf_stats: &mut PerfStats, elapsed: Duration) {
    perf_stats.layers_build_ms += elapsed.as_secs_f32() * 1000.0;
}

pub fn add_layers_draw_time(perf_stats: &mut PerfStats, elapsed: Duration) {
    perf_stats.layers_draw_ms += elapsed.as_secs_f32() * 1000.0;
}
...
pub fn overlay_text(perf_stats: &PerfStats) -> String {
    format!(
        "FPS: {:.1}\nTransform: {:.3} ms\nOutliner: {:.3} build / {:.3} draw ms\nLayers: {:.3} build / {:.3} draw ms\nMesh regen: {}\nGizmo lines: {}",
        perf_stats.fps,
        perf_stats.transform_preview_ms,
        perf_stats.outliner_build_ms,
        perf_stats.outliner_draw_ms,
        perf_stats.layers_build_ms,
        perf_stats.layers_draw_ms,
