...
use super::dependencies::{DependencyRef, DependencyRole};
...
pub enum PackError {
    ...
    AlreadyRegistered { pack_id: PackId, revision: PackRevision },
}
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::CyclicDependency { cycle } => write!(
                f,
                "cyclic dependency: {}",
                cycle.iter().map(|p| p.as_str()).collect::<Vec<_>>().join(" → "),
            ),
            Self::CoreApiMismatch { pack_id, required, actual } => write!(
                f,
                "pack '{}': core_api requires '{required}', running '{actual}'",
                pack_id.as_str(),
            ),
            Self::CapabilityApiMismatch { pack_id, kind, required, actual } => write!(
                f,
...
fn parse_manifest(bytes: &[u8], path: &str) -> Result<PackManifest, PackError> {
    ...
    if is_toml {
        toml::from_str(
            std::str::from_utf8(bytes).map_err(|e| PackError::Parse {
                path: path.to_owned(),
                message: e.to_string(),
            })?,
        )
...
fn check_core_api_compat(manifest: &PackManifest, platform_version: &str) -> Result<(), PackError> {
    ...
    let req = semver::VersionReq::parse(req_str.as_str()).map_err(|_| {
        PackError::CoreApiMismatch {
            pack_id: manifest.pack_id.clone(),
            required: req_str.as_str().to_owned(),
            actual: platform_version.to_owned(),
        }
    })?;
    let ver = semver::Version::parse(platform_version).map_err(|_| {
        PackError::CoreApiMismatch {
            pack_id: manifest.pack_id.clone(),
            required: req_str.as_str().to_owned(),
            actual: platform_version.to_owned(),
...
mod tests {
    ...
    use crate::curation::entitlement::{AlwaysDenyEntitlements, AllowAllEntitlements};
    use crate::curation::identity::{AssetId, AssetKindId, AssetRevision, PackId, PackRevision, SourceId};

    fn minimal_manifest(pack_id: &str, revision: &str) -> PackManifest {
        PackManifest::new(PackId::new(pack_id), PackRevision::new(revision), "test pack")
    }
    ...
    fn registry_register_and_get() {
        ...
        assert!(reg.get(&PackId::new("pack_a"), &PackRevision::new("v1")).is_some());
    }
...
"#;
    ...
    fn load_pack_denied_entitlement_returns_error() {
        ...
        f.write_all(serde_json::to_string(&m).unwrap().as_bytes()).unwrap();

        let mut reg = PackRegistry::default();
        let resolver = AlwaysDenyEntitlements::new("test denial");
        let err = load_pack(&path, "0.1.0", &Actor::new("user:x"), &resolver, &mut reg)
            .unwrap_err();
        assert!(matches!(err, PackError::EntitlementDenied { .. }));
        assert!(reg.get(&PackId::new("gated_pack"), &PackRevision::new("v1")).is_none());
    }
    ...
    fn load_pack_open_succeeds_for_unconstrained_manifest() {
        ...
        let path = dir.path().join("pack.json");
        let mut f = std::fs::File::create(&path).unwrap();
        f.write_all(serde_json::to_string(&minimal_manifest("open", "v1")).unwrap().as_bytes()).unwrap();
