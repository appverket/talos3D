...
use super::authoring_script::{AuthoringScript, McpToolId, MutationScope};
...
impl std::fmt::Display for LlmError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::UngroundedClaim { missing_artifact_kind, .. } => write!(
                f,
...
impl std::fmt::Display for SynthesisError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::UngroundedClaim { missing_artifact_kind, gap_id } => write!(
                f,
...
/// Wraps a pre-approved, guardrail-checked `Vec<McpCall>` as a
/// `ToolDispatcher`. Each `dispatch` call pops the front of the queue,
/// verifies the tool name matches (belt-and-suspenders), and returns the
/// stored response via the inner dispatcher (or `Null` when no inner
/// dispatcher is provided and the call is just replayed for side effects).
struct SynthesisDispatcher<'a, D: ToolDispatcher> {
    /// Guardrail-checked calls in order.
    calls: std::collections::VecDeque<McpCall>,
    /// The real model-api dispatcher that executes the calls.
    inner: &'a mut D,
}

impl<'a, D: ToolDispatcher> SynthesisDispatcher<'a, D> {
    fn new(calls: Vec<McpCall>, inner: &'a mut D) -> Self {
        Self {
...
pub fn synthesize<D: ToolDispatcher, O: PostconditionOracle>(
    ...
    let raw_calls = match llm.draft_calls(&prompt) {
        Ok(calls) => calls,
        Err(LlmError::UngroundedClaim { missing_artifact_kind, context }) => {
    ...
    replay(&synthesized_script, prompt.params, &mut synth_dispatcher, oracle)
...
mod tests {
    use super::*;
    use crate::curation::authoring_script::{MutationScope, Step, StepId, OutputPath};
    use crate::curation::replay::{
        PostconditionOracle, PostconditionVerdict, ResolvedPostcondition, ToolCall, ToolDispatchError,
        ToolDispatcher,
    };
    ...
    fn postcondition_failure_returns_invocation_failed() {
        ...
        script.postconditions.push(Postcondition::Relation {
            ...
            from: ArgExpr::Literal { value: Value::from("a") },
            to: ArgExpr::Literal { value: Value::from("b") },
        });
        let prompt = prompt_for(&script);
    ...
    fn step_budget_exceeded_returns_error() {
        ...
        .unwrap_err();
        assert!(matches!(err, SynthesisError::StepBudgetExceeded { limit: 1 }));
    }
}
