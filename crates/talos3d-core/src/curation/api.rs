...
use super::material_specs::{
    MaterialSpec, MaterialSpecBody, MaterialSpecRegistry, next_material_spec_asset_id,
};
...
mod tests {
    ...
    fn nominate_source_adds_to_queue_without_touching_registry() {
        ...
        assert!(
            reg.get(&SourceId::new("new.source"), &SourceRevision::new("v1"))
                .is_none()
        );
    ...
    fn reject_nomination_drops_without_registry_change() {
        ...
        let reg = app.world().resource::<SourceRegistry>();
        assert!(
            reg.get(&SourceId::new("x"), &SourceRevision::new("v1"))
                .is_none()
        );
    }
