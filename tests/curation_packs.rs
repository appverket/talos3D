...
use talos3d_core::curation::{
    api::{check_pack_compatibility_handler, get_pack_manifest, list_packs, resolve_pack_deps_handler},
    ...
    load_pack, load_pack_open, PackManifest, PackRegistry, PackError,
...
fn write_manifest_json(manifest: &PackManifest, dir: &tempfile::TempDir) -> std::path::PathBuf {
    let path = dir.path().join(format!("{}.json", manifest.pack_id.as_str()));
    let mut f = std::fs::File::create(&path).unwrap();
    f.write_all(serde_json::to_string(manifest).unwrap().as_bytes()).unwrap();
    path
}

fn write_manifest_toml(manifest: &PackManifest, dir: &tempfile::TempDir) -> std::path::PathBuf {
    let path = dir.path().join(format!("{}.toml", manifest.pack_id.as_str()));
    let mut f = std::fs::File::create(&path).unwrap();
    f.write_all(toml::to_string(manifest).unwrap().as_bytes()).unwrap();
    path
}
...
fn load_pack_with_assets_and_source_registers_successfully() {
    ...
    let loaded = registry.get(&PackId::new("test_pack"), &PackRevision::new("v1")).unwrap();
...
fn execution_dep_on_missing_recipe_fails_resolve_but_pack_is_listed() {
    ...
    let ctx = DepResolverCtx { recipes: &recipes, sources: &sources };
...
fn pack_with_incompatible_core_api_fails_compatibility_check() {
    ...
    assert!(
        findings.iter().any(|f| f.is_error() && f.message.contains("core_api")),
...
fn default_entitlement_resolver_allows_all() {
    ...
    m.entitlement = Some(talos3d_core::curation::pack::EntitlementHook::new("some/hook"));
...
fn always_deny_entitlement_blocks_load() {
    ...
    assert!(registry.get(&PackId::new("denied_pack"), &PackRevision::new("v1")).is_none());
}
...
fn api_check_pack_compatibility_returns_error_for_future_core_api() {
    ...
    assert!(findings.iter().any(|f| f.code == "curation.pack.core_api_mismatch"));
}
