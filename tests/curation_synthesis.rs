...
use talos3d_core::curation::{
    authoring_script::{
        ArgExpr, AuthoringScript, McpToolId, MutationScope, OutputPath, Postcondition, Step,
        StepId,
    },
...
fn out_of_scope_tool_call_rejected_before_dispatch() {
    ...
    assert!(matches!(err, SynthesisError::EmittedCallToolNotAllowed { .. }));
    assert_eq!(
...
fn ungrounded_claim_enqueues_corpus_gap_and_returns_error() {
    ...
    match err {
        SynthesisError::UngroundedClaim { missing_artifact_kind, gap_id } => {
...
fn step_budget_exceeded_before_dispatch() {
    ...
    assert!(matches!(err, SynthesisError::StepBudgetExceeded { limit: 3 }));
